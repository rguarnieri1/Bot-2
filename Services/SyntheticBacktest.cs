...
public class SyntheticBacktest
{
    ...
    public void PrintBacktestReport(SyntheticBacktestResult result)
    {
        ...
        var smaCount = result.Trades.Count(t => t.Strategy.Contains("SMA"));
        if (result.Trades.Count > 0)
            Console.WriteLine($"   • SMA Strategy: {smaCount} ({(decimal)smaCount / result.Trades.Count * 100:F1}%)");
